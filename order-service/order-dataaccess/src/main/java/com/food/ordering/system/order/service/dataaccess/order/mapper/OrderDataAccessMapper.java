...
public class OrderDataAccessMapper {
  ...
  public Order orderEntityToOrder(OrderEntity orderEntity) {
    return Order.builder()
        ...
        .items(orderEntitiesToOrderItem(orderEntity.getItems()))
        .trackingId(new TrackingId(orderEntity.getTrackingId()))
        .failureMessages(orderEntity.getFailureMessages()
            .isEmpty() ? new ArrayList<>() : new ArrayList<>(Arrays.asList(orderEntity.getFailureMessages()
            .split(FAILURE_MESSAGE_DELIMITER))))
        .build();
  }

  private List<OrderItem> orderEntitiesToOrderItem(List<OrderItemEntity> items) {
    return items.stream()
